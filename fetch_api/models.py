...
# Por questão de segurança, os dados sensíveis que retornam da api randomuser
# não foram persistidos.
# api envia senha em texto plano, o salt e os hashs das senhas
# (md5, sha1, sha256) - não salvos.

# mantive os nomes em inglês no modelo em conformidade com a api consumida.
# não fiz relações um para muitos com FKs porque cada usuario persistido
# contém um endereço já em seu objeto.
# se houver interesse em um cadastro de endereços unicos para relacionar
# com mais de um usuario, é possível alterar.
...
class Location(models.Model):
    ...
    # adicionei pois achei interessante usar DecimalField com decimal_places
...
class UserInformation(models.Model):
    # relacao um para um com UserProfile
    user_profile = models.OneToOneField(UserProfile, on_delete=models.CASCADE)
    # relacao um para um com Location
    location = models.OneToOneField(Location, on_delete=models.CASCADE)
    phone = models.CharField(max_length=20)
    # Celular pode ser opcional
